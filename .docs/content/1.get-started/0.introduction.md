...
## What is `@j-stream/providers`?

`@j-stream/providers` is the soul of [j-stream](https://github.com/j-stream/j-stream). It's a collection of scrapers of various streaming sites. It extracts the raw streams from those sites, so you can watch them without any extra fluff from the original sites.
