...
async function comboScraper(ctx: ShowScrapeContext | MovieScrapeContext): Promise<SourcererOutput> {
  // Strip 'tt' prefix if present to get numeric-only IMDb ID
  const rawImdbId = ctx.media.imdbId;
  if (!rawImdbId) throw new NotFoundError('IMDB ID required');
  const cleanImdbId = rawImdbId.replace(/^tt/, '');

  // Build the API URL based on media type
  let apiUrl: string;
  if (ctx.media.type === 'movie') {
    apiUrl = `https://primewire.pw/movie/${cleanImdbId}`;
  } else {
    apiUrl = `https://primewire.pw/tv/${cleanImdbId}/${ctx.media.season.number}/${ctx.media.episode.number}`;
  }
